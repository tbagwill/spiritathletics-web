...
export default function RegisterClinicDialog({ clinicId, clinicTitle, clinicWhen, priceCents, spotsLeft }: Props) {
  ...
  const addAthlete = () => {
    if (numAthletes < spotsLeft) setAthleteNames([...athleteNames, '']);
  };
      ...
      if (paymentMethod === 'CASH') {
        const res = await fetch('/api/clinics/register', {
          ...
          body: JSON.stringify({
            ...
            athleteFirstNames: trimmedNames,
          }),
        });
        const data = await res.json();
        if (!res.ok || !data.ok) throw new Error(data.error || 'Registration failed');
        window.location.href = `/book/success?type=clinic&payment=cash`;
      } else {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10 max-h-[90vh] overflow-y-auto text-gray-900">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <svg className={`w-6 h-6 ${paymentMethod === 'CARD' ? 'text-purple-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                    <span className={`text-sm font-semibold ${paymentMethod === 'CARD' ? 'text-purple-700' : 'text-gray-700'}`}>Pay Online</span>
                    <span className={`text-xs ${paymentMethod === 'CARD' ? 'text-purple-500' : 'text-gray-400'}`}>
                      + {feeText} processing
                    </span>
                  </button>
              ...
              {/* Price Summary */}
              <div className={`rounded-xl p-4 border ${paymentMethod === 'CASH' ? 'bg-green-50 border-green-100' : 'bg-purple-50 border-purple-100'}`}>
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-sm">
                    <span className={paymentMethod === 'CASH' ? 'text-green-800' : 'text-purple-800'}>
                      {numAthletes > 1 ? `${numAthletes} athletes × ${perAthleteText}` : `Clinic registration`}
                    </span>
                    <span className={`font-semibold ${paymentMethod === 'CASH' ? 'text-green-800' : 'text-purple-800'}`}>
                      {numAthletes > 1 ? `$${(baseTotalCents / 100).toFixed(2)}` : perAthleteText}
                    </span>
                  </div>
                  {paymentMethod === 'CARD' && (
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-purple-600">Card processing fee</span>
                      <span className="text-purple-600">{feeText}</span>
                    </div>
                  )}
                  <div className="border-t border-current/10 pt-1.5 flex justify-between items-center">
                    <span className={`text-sm font-semibold ${paymentMethod === 'CASH' ? 'text-green-900' : 'text-purple-900'}`}>Total</span>
                    <span className={`text-lg font-bold ${paymentMethod === 'CASH' ? 'text-green-700' : 'text-purple-900'}`}>
                      ${(displayTotal / 100).toFixed(2)}
                    </span>
                  </div>
                </div>
                <p className={`text-xs mt-2 ${paymentMethod === 'CASH' ? 'text-green-600' : 'text-purple-600'}`}>
              ...
              <div className="flex items-center justify-end gap-3 pt-1">
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      {paymentMethod === 'CASH' ? 'Register' : 'Pay'} — ${(displayTotal / 100).toFixed(2)}
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-xs text-gray-400 text-center">Your athlete{numAthletes > 1 ? 's\'' : '\'s'} first name{numAthletes > 1 ? 's' : ''} will be listed publicly as registered participant{numAthletes > 1 ? 's' : ''}.</p>
            </form>
          </div>
        </div>
      )}
